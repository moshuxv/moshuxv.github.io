(function () {
    ...
    async function init() {
        try {
            ...
            // 然后加载后续内容
            const data = await fetch('src/assets/1.json').then(res => {
                if (!res.ok) {
            ...
            for (let i = 0; i < LINES.length; i++) {
                ...
                if (!el) {
                    throw new Error(`Element with id ${line.id} not found.`);
                }
    ...
    async function handleAdditionalLinesInLine3() {
        ...
        if (!line3El) {
            throw new Error('Element with id line3 not found.');
        }
    ...
    function addBlinkingCursor(lineId) {
        ...
        if (!lastLine) {
            throw new Error(`Element with id ${lineId} not found.`);
        }
